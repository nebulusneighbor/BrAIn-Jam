...
for condition, (start, end) in conditions.items():
    ...
    if start in df[0].values and end in df[0].values:
        ...
        # Create a time array (in seconds) for the x-axis of the plot
        time = np.arange(0, len(condition_data)*0.2, 0.2)

        # Plot the data over time
        plt.plot(time, condition_data)
        plt.title(f'{condition} Over Time')
        plt.xlabel('Time (s)')
        plt.ylabel('Value')
        plt.show()
    else:
